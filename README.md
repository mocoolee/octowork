...
**Q：我的資料安全嗎？**
A：你的 OctoWork 檔案都存在本地的 `workspace/` 裡，不會上傳到任何地方。

**Q：升級會不會覆蓋我的東西？**
A：不會。`workspace/` 被 `.gitignore` 保護，`git pull` 只會更新框架部分。

**Q：可以給團隊裡的多個人用嗎？**
A：可以。每個人在自己的 `workspace/` 裡建自己的 OctoWork，共用同一套框架。詳見 [團隊部署](docs/team-deployment.md)。
