...
<details>
<summary><strong>我沒有裝 Node.js 怎麼辦？</strong></summary>

到 [nodejs.org](https://nodejs.org/) 下載 LTS 版本，安裝時一直按「下一步」就好。裝完後打開終端機輸入 `node -v`，看到版本號就代表成功。

**什麼是終端機？** Mac 按 `Cmd + 空白鍵` 搜尋「Terminal」。Windows 按 `Win` 搜尋「PowerShell」。
</details>

<details>
<summary><strong>安裝 Claude Code / Gemini CLI 失敗？</strong></summary>

| 錯誤訊息 | 原因 | 解法 |
|----------|------|------|
| `command not found: npm` | Node.js 沒裝好 | 重裝 Node.js |
| `permission denied` | Mac 權限問題 | 指令前面加 `sudo` |
| `npm ERR! network` | 網路問題 | 換個網路或稍後再試 |
| `claude: command not found` | 安裝沒成功 | 重跑 `npm install -g @anthropic-ai/claude-code` |
</details>
