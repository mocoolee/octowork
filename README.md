...
#### 選項 C：OpenAI Codex CLI

```bash
npm install -g @openai/codex
```

安裝完輸入 `codex`，用 ChatGPT Plus / Pro 帳號登入。

#### 選項 D：VS Code + AI 擴充套件

如果你習慣用 VS Code，不需要裝 CLI 工具。直接安裝以下任一擴充套件：
- [Claude for VS Code](https://marketplace.visualstudio.com/items?itemName=anthropics.claude-code) — Anthropic 官方
- [Copilot](https://marketplace.visualstudio.com/items?itemName=GitHub.copilot) — GitHub / OpenAI

在 VS Code 裡打開 octowork 資料夾，用擴充套件的對話視窗跟 AI 互動即可。

#### 選項 E：Google Antigravity

[Antigravity](https://antigravity.google/) 是 Google 基於 VS Code 架構的 AI 開發平台，由 Gemini 驅動。直接在 Antigravity 裡打開 octowork 資料夾就能用。
...
每個月跟 AI 說「復盤」，AI 會帶你走五個階段。整個過程大約 15-20 分鐘。
...
有的話就講，AI 會一直問「還有嗎？」直到你說沒了。

如果一時想不到，AI 會用四個方向引導你：
1. 最近什麼事情最花你時間？
2. 有沒有什麼事做完常常要回頭改？
3. 有沒有什麼事你一直排不進去？
4. 有沒有什麼事只有你會做，別人接不了？
...
如果真的都沒有，你可以選：
1. **讓 AI 檢查系統** — AI 自己掃全系統找問題
2. **補全資料** — 一起把公司、產品、協作者的資料補完整
3. **跳過** — 這次不復盤

### Phase 2：針對痛點解決

AI 不會聽到痛就直接改，而是先追問「為什麼」：

> 「你說報價很花時間，是因為格式要調？資料要查很久？還是每次都要重新講一遍？」

找到根本原因再對症下藥：
- 花太多時間 → 優化 Skill 的產出模板或輸入格式
- 常出錯 → 補品質規範和判斷邏輯
- 一直拖 → 簡化流程或拆步驟
- 只有你會做 → 存成 Skill
...
你的痛點處理完後，AI 會根據檔案現況，挑最需要的面向主動問你（不是全部都問）。

<details>
<summary>AI 會檢查哪些面向</summary>
...
**技能（三層深度）：**
- 缺什麼：有沒有重複工作還沒存成 Skill？
- 改什麼：產出模板要調嗎？品質規範要補嗎？輸入容錯夠嗎？需要精簡版嗎？
- AI 主動建議：「你的 XX Skill 沒有精簡版，要不要加？」「你的 XX Skill 只接受表格，要不要加口述輸入？」

**流程（三層深度）：**
- 缺什麼：有沒有常連著用的 Skill 該串成 Workflow？
- 改什麼：哪一步最常卡住？步驟順序對嗎？
- 加速：哪一步每次都直接確認可以拿掉？哪一步該加確認點？
...
- 清完過時資訊
- 列出這次復盤更新了什麼
- 預告下次復盤會關注什麼（例如：「你這次新建了月結報表 Skill，下次我會問你用得順不順」）
...
| [OpenAI Codex CLI](https://github.com/openai/codex) | ✅ | 支援 AGENTS.md + .codex/skills/ |
| [VS Code + AI 擴充](https://code.visualstudio.com/) | ✅ | Claude / Copilot 擴充套件，打開資料夾就能用 |
| [Google Antigravity](https://antigravity.google/) | ✅ | 基於 VS Code，Gemini 驅動，打開資料夾就能用 |
