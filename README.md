...
AI 會讀完你所有的檔案，然後根據現有內容找出可以補強的地方——做事原則太少的工作類型、沒寫溝通風格的協作者、缺 Skill 的工作、可以串成 Workflow 的組合。一次問你一個問題，你回答後 AI 直接幫你更新檔案。說「夠了」就結束。
...
大約 10-15 分鐘。AI 會用聊天的方式問你四輪問題：

1. **你是誰**（2-3 題）— 公司、職稱、KPI、團隊、工具
2. **你怎麼做事**（2-3 題）— 判斷依據、主管偏好、踩過的坑
3. **你的重複工作**（2-3 題）— 每天/每週/每月固定做的事
4. **你的流程**（1-2 題）— 做完 A 就要做 B 再做 C 的串接工作

像跟同事聊天一樣，不用提前準備。每一輪結束 AI 會摘要確認，你覺得 OK 才往下走。聊完之後 AI 會自動生成你的完整系統（CLAUDE.md + company.md + products.md + 5-7 個 Skills + 2-4 個 Workflows）。
</details>
...
每個人的 Skill 只屬於他自己的系統，不會影響別人，也不會跑到 Claude Code 的全域路徑（`~/.claude/`）。系統規則裡有明確指示 AI 只能存到本資料夾的 `.agents/skills/`。
</details>

<details>
<summary><strong>做事原則越來越多怎麼辦？</strong></summary>

建議控制在 30 條以內。超過 30-40 條，AI 反而抓不到重點，可能會忽略真正重要的原則。
...
AI 會讀完你系統裡的所有檔案（CLAUDE.md、company.md、products.md、所有 Skills 和 Workflows），然後根據現有內容找出可以補強的地方，一次問你一個延伸問題。

它會問的方向包括：

- 某個工作類型的做事原則太少，能不能補更多？
- 某個協作者只有名字沒有溝通風格，他偏好什麼？
- 你的做事原則提到某件工作，但沒有對應的 Skill，要不要建一個？
- 有幾個 Skill 看起來常常一起用，要不要串成 Workflow？
- company.md 或 products.md 有沒有缺漏的資訊？

你回答後，AI 直接幫你更新對應的檔案。覺得夠了就說「夠了」，AI 會摘要這次復盤更新了什麼。

建議用了一兩週後做一次復盤，效果最好——因為你已經知道哪些 Skill 好用、哪些還不夠。
</details>
...
**越做越聰明。** 做完事 → AI 問你「學到什麼」→ 更新做事原則 → 下次做得更好。做事原則累積太多？每月跟 AI 說「整理原則」就好。詳見 [維護指南](docs/maintaining-your-system.md)。
